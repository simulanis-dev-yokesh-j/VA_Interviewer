...
public class HelloWorld : MonoBehaviour
{
     ...
     **********************************/
    // Use cloud-based Azure Speech Service instead of embedded models
    private static readonly string subscriptionKey = "9v2mwusnyHO6LBsDWvcc6xh6M9WsiJIuqEbwe95kHTABg7bAiEs3JQQJ99BFACGhslBXJ3w3AAAYACOGOxq2";
    private static readonly string region = "centralindia";
    
    // NOTE: Embedded speech is disabled - using cloud service instead
    // If you want to use embedded speech, you need:
    // 1. Download offline models from Azure Speech Studio
    // 2. Get valid license strings for your models
    // 3. Configure the model names and file paths below
    ...
    public void OnInitObjectsButtonClicked()
    {
        try
        {
            ...
            config.SpeechRecognitionLanguage = "en-US"; // Keep recognition as US English for better accuracy
            config.SpeechSynthesisLanguage = "en-IN";   // Indian English for synthesis
            config.SpeechSynthesisVoiceName = "en-IN-AaravNeural"; // Indian male neural voice

            // Set high-quality audio format for cloud synthesis
            config.SetSpeechSynthesisOutputFormat(SpeechSynthesisOutputFormat.Riff24Khz16BitMonoPcm);

            StringBuilder sb = new StringBuilder();
            sb.Append("Initialized");

            // Creates a speech recognizer instance using cloud service
            if (!string.IsNullOrEmpty(subscriptionKey) && !string.IsNullOrEmpty(region))
            {
                recognizer = new SpeechRecognizer(config);
                sb.Append(" recognizer");
                
                // Also create synthesizer since we're using cloud service
                synthesizer = new SpeechSynthesizer(config);
                sb.Append(" and synthesizer");
            }
    ...
    void Start()
    {
        UnityEngine.Debug.Log($"CloudSpeechSamples using Azure Speech Service");
        UnityEngine.Debug.Log($"SubscriptionKey configured: {!string.IsNullOrEmpty(subscriptionKey)}");
        UnityEngine.Debug.Log($"Region: {region}");
        ...
        else
        {
...
#endif
            enableCopyFilesButton = false; // Not needed for cloud service
            enableInitObjectsButton = true;
            enableRecognitionButton = true;
            enableSynthesisButton = true;
            ...
            statusMessage = "Welcome to the Cloud Speech sample for Unity! Click 'Init Objects' to start.";
        }
    }
