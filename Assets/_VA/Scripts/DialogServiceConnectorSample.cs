...
{
    private string subscriptionKey = "9v2mwusnyHO6LBsDWvcc6xh6M9WsiJIuqEbwe95kHTABg7bAiEs3JQQJ99BFACGhslBXJ3w3AAAYACOGOxq2";
    private string region = "centralindia";
    private string botId = "f30d525d-c49b-4b7b-a86d-aa3b7776dd97";
    ...
    [Header("Interview Bot")]
    public InterviewBotLogic interviewBot;
    ...
    private void CreateDialogServiceConnector()
    {
        ...
        if (dialogServiceConnector == null)
        {
            if (subscriptionKey == string.Empty || region == string.Empty || botId == string.Empty)
            {
            ...
            // For testing, you can try using BotFrameworkConfig with your subscription key and region
            // Make sure you have created a bot in Azure Bot Service with Direct Line Speech channel enabled
            
            // If you have a specific bot ID, replace "YOUR_BOT_ID" with your actual bot ID
            dialogServiceConfig = BotFrameworkConfig.FromSubscription(subscriptionKey, region, botId);
            
            // For now, using without bot ID (requires default bot to be set up)
    ...
    private void DialogServiceConnector_Recognized(object sender, SpeechRecognitionEventArgs e)
    {
        ...
        if (e.Result.Reason == ResultReason.RecognizedSpeech)
        {
            ...
            lock (threadLocker)
            {
                ...
                // Process speech input with interview bot
                if (interviewBot != null)
                {
                    string userInput = e.Result.Text.ToLower();
                    
                    if (userInput.Contains("start interview") && !interviewBot.IsInterviewCompleted())
                    {
                        recognizedString = interviewBot.StartInterview();
                    }
                    else if (!interviewBot.IsInterviewCompleted())
                    {
                        recognizedString = interviewBot.ProcessUserResponse(e.Result.Text);
                    }
                    else
                    {
                        recognizedString = "Interview completed. Thank you!";
                    }
                }
            }
        }
